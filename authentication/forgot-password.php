...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'request') {
        ...
        if ($user) {
            ...
            $conn->prepare('INSERT INTO password_resets (email, token, expires_at) VALUES (?, ?, DATE_ADD(NOW(), INTERVAL 1 HOUR))')->execute([$email, $token]);
            ...
            if (@mail($to, $subject, $message, $headers)) {
                $success = 'A password reset link has been successfully sent to your email address. Please check your inbox.';
            } else {
                // Fallback for systems without working sendmail/SMTP configuration
                $success = 'A password reset link was generated for your account. (Notice: Mail utility failed to deliver. Reset URL: ' . $resetLink . ')';
            }
        } else {
            $error = 'No registered user account found with that email address.';
        }
    }
