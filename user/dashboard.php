...
$upcoming = $conn->prepare("SELECT COUNT(*) FROM bookings WHERE user_id = ? AND status IN ('pending','confirmed') AND check_in >= CURDATE()");
...
$list = $conn->prepare("SELECT b.*, h.title, h.city, h.cover_image FROM bookings b JOIN homestays h ON h.id = b.homestay_id
    WHERE b.user_id = ? AND b.status IN ('pending','confirmed') AND b.check_out >= CURDATE()
