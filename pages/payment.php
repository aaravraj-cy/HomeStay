...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($method === 'card' && !preg_match('/^\d{16}$/', $cardNum)) {
        $error = 'Enter a valid 16-digit card number (demo).';
    } elseif ($method === 'upi' && empty($_POST['upi_id'])) {
        $error = 'Enter a valid UPI ID (demo).';
    } elseif (!is_room_available($pending['room_id'], $pending['check_in'], $pending['check_out'])) {
...
<div class="page-header-bar animate__animated animate__fadeIn">
    <div class="container">
        <h1 class="display-font fw-bold text-teal-deep mb-1">Complete Secure Payment</h1>
        <p class="text-muted mb-0">Booking Reference &bull; Secure Encrypted Checkout</p>
    </div>
</div>
            ...
            <div class="dash-card border p-4">
                ...
                <form method="POST" id="paymentForm">
                    ...
                    <button type="submit" class="btn btn-primary w-100 py-2.5 fw-bold d-flex align-items-center justify-content-center gap-2">
                        <i class="fas fa-shield-halved"></i> Authorize & Pay <?= money($pending['total_amount']) ?>
                    </button>
                </form>
            </div>
        </div>
        ...
        <div class="col-lg-5">
            <div class="booking-widget border rounded-4 p-4 shadow-sm bg-white">
                ...
                <div class="d-flex gap-2 align-items-start bg-light p-2.5 rounded-3 mt-3">
                    ...
                    <p class="small text-muted mb-0" style="line-height: 1.4">This is a student demo transaction. Card inputs do not store real financial data.</p>
                </div>
            </div>
        </div>
    </div>
