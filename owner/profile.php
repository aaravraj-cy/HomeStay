    ...
    <div class="dashboard-main animate__animated animate__fadeIn">
        ...
        <div class="d-flex border-bottom gap-4 mb-4" style="max-width: 650px">
            <button class="btn btn-link nav-link pb-2.5 fw-bold px-0 border-bottom border-2 <?= $activeTab === 'profile' ? 'border-teal text-teal' : 'border-transparent text-muted' ?>" id="tabProfile" type="button">
                <i class="far fa-user me-1.5"></i> Profile Details
            </button>
            <button class="btn btn-link nav-link pb-2.5 fw-bold px-0 border-bottom border-2 <?= $activeTab === 'security' ? 'border-teal text-teal' : 'border-transparent text-muted' ?>" id="tabSecurity" type="button">
                <i class="fas fa-lock me-1.5"></i> Security
            </button>
        </div>
        ...
        <div class="tab-pane <?= $activeTab === 'profile' ? '' : 'd-none' ?>" id="paneProfile">
            <form method="POST" enctype="multipart/form-data" class="dash-card border p-4" style="max-width: 650px">
                ...
                <!-- Full Name -->
                <div class="mb-3">
                    <label class="form-label small fw-bold text-muted text-uppercase mb-1"><i class="far fa-user text-teal me-1.5"></i>Full name</label>
                    <input name="full_name" class="form-control fw-semibold" value="<?= e($user['full_name']) ?>" required>
                </div>
                
                <!-- Email (Disabled) -->
                <div class="mb-3">
                    <label class="form-label small fw-bold text-muted text-uppercase mb-1"><i class="far fa-envelope text-teal me-1.5"></i>Email Address</label>
                ...
                <!-- Phone -->
                <div class="mb-3">
                    <label class="form-label small fw-bold text-muted text-uppercase mb-1"><i class="fas fa-phone text-teal me-1.5"></i>Phone number</label>
                    <input name="phone" class="form-control" placeholder="Enter contact phone" value="<?= e($user['phone'] ?? '') ?>">
                </div>
                
                <!-- Business Name -->
                <div class="mb-3">
                    <label class="form-label small fw-bold text-muted text-uppercase mb-1"><i class="fas fa-briefcase text-teal me-1.5"></i>Business / Homestay name</label>
                    <input name="business_name" class="form-control" placeholder="e.g. Sikkim Mountain Homestays" value="<?= e($owner['business_name'] ?? '') ?>">
                </div>
                
                <!-- Bio -->
                <div class="mb-4">
                    <label class="form-label small fw-bold text-muted text-uppercase mb-1"><i class="far fa-note-sticky text-teal me-1.5"></i>Host Bio</label>
                ...
                <button class="btn btn-teal py-2 px-4 fw-bold"><i class="fas fa-save me-1.5"></i>Save Details</button>
            </form>
        </div>
        ...
        <div class="tab-pane <?= $activeTab === 'security' ? '' : 'd-none' ?>" id="paneSecurity">
            <form method="POST" class="dash-card border p-4" style="max-width: 650px">
                ...
                <button class="btn btn-teal py-2 px-4 fw-bold"><i class="fas fa-lock me-1.5"></i>Change Password</button>
            </form>
        </div>
